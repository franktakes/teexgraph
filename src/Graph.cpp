...
bool Graph::loadDirected(const string filename) {
    ...
    ifstream fin;
    //FILE * fin; // C-style file handling
    ...
    //	fin = fopen(filename.c_str(), "r");  // C-style file handling

    // ignore first lines that do not start with an integer character  on C-style file handling
    /*char c = getc(fin);
    while (c < '0' || c > '9') {
        while (c != '\n') 
            c = getc(fin);
    ...
    // ignore first lines that do not start with an integer character  on C-style file handling
    // dont do this for nodetype string
    char c = fin.peek();
    while(c < '0' || c > '9'){
        while(c != '\n')
            c = fin.get();
    ...
    // load the edge list
    //while (fscanf(fin, "%d %d", &u, &v) > 0) { // use %u for string, C-style
    while(fin >> u >> v){
...
// print some statistics about the currently loaded graph.
void Graph::stats() {
    const int LEFTWIDTH = 40;
    clog << setw(LEFTWIDTH) << "Nodes n: " << nodes(FULL) << endl;
    clog << setw(LEFTWIDTH) << "Links m: " << edges(FULL) << endl;
    clog << setw(LEFTWIDTH) << "Self-edges: " << selfEdges(FULL) << endl;
    if(undirected)
        clog << setw(LEFTWIDTH) << "Undirected edges: " << selfEdges(FULL) + ((edges(FULL) - selfEdges(FULL)) / 2) << endl;
    clog << setw(LEFTWIDTH) << "Average degree: " << averageDegree(FULL) << endl;
    clog << setw(LEFTWIDTH) << "Density: " << density(FULL) << endl;
    if(!undirected)
        clog << setw(LEFTWIDTH) << "Reciprocity: " << reciprocity(FULL) << endl;

    //clog << setw(LEFTWIDTH) << "Triangles: " << triangles() << endl; 
    //clog << setw(LEFTWIDTH) << "Wedges: " << wedges() << endl; 
    //clog << setw(LEFTWIDTH) << "Graph clustering coefficient: " << graphClusteringCoefficient(false) << endl; 
    //clog << setw(LEFTWIDTH) << "Average local clustering coefficient: " << averageClusteringCoefficient(false) << endl; 
    //clog average distance

    clog << endl;
    clog << setw(LEFTWIDTH) << "Sorted & unique edge lists: " << (sortedandunique ? "Yes" : "No") << endl;
    clog << setw(LEFTWIDTH) << "Undirected/symmetric edge set: " << (undirected ? "Yes" : "No") << endl;
    clog << setw(LEFTWIDTH) << "WCC computed: " << (doneWCC ? "Yes" : "No") << endl;
    clog << setw(LEFTWIDTH) << "SCC computed: " << (doneSCC ? "Yes" : "No") << endl;
    clog << endl;
